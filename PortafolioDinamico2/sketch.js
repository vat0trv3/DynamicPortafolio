...
      { name: "Investigación y Desarrollo en Colaboración con IA ", points: 
       
       ["Desarrollo proactivo de competencias en Creative Coding a través de la colaboración con un asistente de IA.", "Creación de prototipos funcionales en Python, como traductores de texto a código Morse y generadores de señales de audio (WAV/MIDI), optimizando flujos de trabajo y acelerando el aprendizaje de nuevas tecnologías."] },
...
// EL RESTO DEL CÓDIGO (LÓGICA, INTERACCIÓN, DIBUJO)
...
// NUEVA FUNCIÓN: genera la lista de conexiones
...
// MODIFICACIÓN: Declaramos totalContentHeight aquí para que se actualice dinámicamente
...
    text(cvData.header.name, centerX, y, w);
...
    text(cvData.header.title, centerX, y, w);
...
    text(cvData.header.contact.location, centerX, y, w);

    y += 30 * 1.30;

    text(cvData.header.contact.email, centerX, y, w);
...
    for (let link of cvData.header.contact.socialLinks) {
...
    text(cvData.profile.title, centerX, y, w);
...
    text(cvData.profile.text, centerX, y, w);

    y += textHeight(cvData.profile.text, w) + 100 * 1.30;
...
    text(cvData.specialization.title, centerX, y, w);

    y += 50 * 1.30;

    

    for(let area of cvData.specialization.areas){
...
        y += textHeight(area.desc, w) + 30 * 1.30;
...
    text(cvData.toolbox.title, centerX, y, w);

    y += 112 * 1.30; // 🎯 Aumentamos el espacio después del título



    for(const category in cvData.toolbox.categories){
...
        y += 45 * 1.30; // 🎯 Espacio después de la categoría
...
        for(const tool of cvData.toolbox.categories[category]){
...
    text(cvData.experience.title, centerX, y, w);

    y += 90 * 1.80;



    for (let project of cvData.experience.projects) {
...
    text(cvData.thinkBig.title, centerX, y, w);
...
    text(cvData.thinkBig.projectTitle, centerX, y, w);

    y += 40 * 1.40; // 🎯 Ajuste: Espacio más adecuado después del título del proyecto



    const sections = [cvData.thinkBig.concepto, cvData.thinkBig.propuesta, cvData.thinkBig.pruebas, cvData.thinkBig.impacto];
...
        y += 40 * true; // 🎯 Ajuste: Espacio reducido para conectar el subtítulo con el texto
...
        y += 50 * 1.40; // Espacio entre cada subsección
...
    text(cvData.thinkBig.conclusion, centerX, y, w);

    y += textHeight(cvData.thinkBig.conclusion, w) + 100 * 1.30;
...
        touchStartY = mouseY; scrollY_atTouchStart = scrollY; handleInteraction(mouseX, mouseY);
...
  if (!isPaused) { scrollY += event.delta; scrollY = constrain(scrollY, 0, totalContentHeight - height); }
...
  if (!isPaused) { let deltaY = mouseY - touchStartY; scrollY = scrollY_atTouchStart - deltaY; scrollY = constrain(scrollY, 0, totalContentHeight - height); }
...
        if(isPaused) { if(demoSound.isPlaying()) demoSound.pause(); } else { if(!demoSound.isPlaying()) demoSound.loop(); }
...
            if (el.type === 'link') { window.open(el.data, '_blank'); return; }

            if (el.type === 'skill' && !demoSound.isPlaying()) { demoSound.loop(); return; }
...
    noStroke(); fill(255, 50); ellipse(40, 40, 50, 50);
...
    let screenTop = scrollY; let screenBottom = scrollY + height;
...
    let words = txt.split(' '); let line = ''; let h = textSize();

    for (let i = 0.; i < words.length; i++) {

        let testLine = line + words[i] + ' '; let testWidth = textWidth(testLine);

        if (testWidth > textWidthVal && i > 0) {

            line = words[i] + ' '; h += textSize();
